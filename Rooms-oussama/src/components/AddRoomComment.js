...
export default function AddComment(props) {
    ...
    useEffect(() => {
        const fetchPosts = async () => {
          const res = await axios.get("http://localhost:5000/api/roompost/" + props.post._id);
    ...
    const handleComment = async (e)=>{
        ...
        await axios.put("http://localhost:5000/api/roompost/"+ props.post._id, {...props.post, comments:[...res.data.comments,{userId:user._id,username:user.username, postId:props.post._id, content:content.current.value,date:new Date(),likes:[], dislikes:[]}]})
        props.handleA()
    }
    return(
